...
def mock_client(mock_auth_response):
    with patch("quotientai.client._BaseQuotientClient") as MockClient, patch(
        "quotientai.resources.AuthResource"
    ) as MockAuthResource:
...
class TestBaseQuotientClient:
    ...
    def test_initialization(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            assert (
                client._token_path
                == tmp_path / ".quotient" / f"{api_key[-6:]}_auth_token.json"
            )
    ...
    def test_handle_jwt_response(self):
        ...
        with patch("jwt.decode") as mock_decode, patch.object(
            _BaseQuotientClient, "_save_token"
        ) as mock_save_token:
    ...
    def test_save_token(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            token_file = (
                tmp_path / ".quotient" / f"{client.api_key[-6:]}_auth_token.json"
            )
    ...
    def test_token_path_uses_home(self):
        with patch("pathlib.Path.home") as mock_home:
            ...
            assert client._token_path == Path(
                "/home/user/.quotient/st-key_auth_token.json"
            )
    ...
    def test_token_path_fallback_to_cwd(self):
        with patch("pathlib.Path.home") as mock_home, patch(
            "pathlib.Path.exists"
        ) as mock_exists, patch("pathlib.Path.cwd") as mock_cwd:
            # Simulate home directory access failing
            mock_home.side_effect = Exception("Can't access home")
            ...
            client = _BaseQuotientClient("test-key")
            assert client._token_path == Path(
                "/current/dir/.quotient/st-key_auth_token.json"
            )
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            with patch.object(client, "get") as mock_get:
                ...
                mock_get.assert_called_with(
                    "/test", params={"key": "value"}, timeout=30
                )
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            with patch.object(client, "post") as mock_post:
                ...
                mock_post.assert_called_with(
                    url="/test", json={"key": "value"}, timeout=30
                )
                ...
                mock_post.assert_called_with(
                    url="/test", json=["value", "value2"], timeout=None
                )
...
class TestQuotientAI:
    ...
    def test_init_auth_failure(self, caplog):
        ...
        with patch("quotientai.resources.AuthResource") as MockAuthResource:
            mock_auth = MockAuthResource.return_value
            mock_auth.authenticate = Mock(
                side_effect=Exception("'Exception' object has no attribute 'message'")
            )

            QuotientAI(api_key="test-api-key")
...
class TestQuotientLogger:
    ...
    def test_invalid_sample_rate(self, caplog):
        ...
        result = logger.init(app_name="test-app", environment="test", sample_rate=2.0)
    ...
    def test_log_without_init(self, caplog):
        ...
        result = logger.log(user_query="test query", model_output="test output")
    ...
    def test_log_with_valid_documents(self):
        ...
        assert calls[0][1]["documents"][1] == {
            "page_content": "dict document",
            "metadata": {"key": "value"},
        }
