...
print("Check URL: ", os.getenv("SUPABASE_URL"))
...
def test_initial_status():
    ...
    assert (
        status["active_login"] is False
    ), "Expected status to show active_login as False"
...
def test_revoked_key_list_failure():
    ...
    assert result is not None, "Expected tasks to be returned"
    assert isinstance(result, list), "Expected tasks to be a list"
    assert len(result) == 0, "Expected no tasks to be returned with invalid API key"
