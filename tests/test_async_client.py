...
class TestAsyncBaseQuotientClient:
    ...
    def test_handle_jwt_response(self):
        ...
        with patch("jwt.decode") as mock_decode, patch.object(
            _AsyncQuotientClient, "_save_token"
        ) as mock_save_token:
    ...
    def test_token_directory_creation_failure(self, tmp_path, caplog):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path), patch.object(
            Path, "mkdir", side_effect=Exception("Test error")
        ):
            client = _AsyncQuotientClient(api_key)
    ...
    async def test_get_wrapper(self, tmp_path):
        ...
        with patch("pathlib.Path.home", return_value=tmp_path):
            ...
            client.get = AsyncMock(
                return_value=Mock(json=Mock(return_value=test_response))
            )
...
class TestAsyncQuotientLogger:
    ...
    async def test_log_without_init(self, caplog):
        ...
        result = await logger.log(user_query="test query", model_output="test output")
