...
def test_no_duplicate_system_prompt(quotient_client):
    with pytest.raises(QuotientAIException) as exc_info:
        quotient_client.create_system_prompt(
            os.getenv("TEST_CREATE_SYSTEM_PROMPT"), "New system prompt"
        )
    assert "System prompt with the same name already exists" in str(
        exc_info.value
    ), "Expected system prompt creation to fail with duplicate name"
