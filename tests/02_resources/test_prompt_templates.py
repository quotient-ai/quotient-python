...
def test_no_duplicate_prompt_template(quotient_client):
    with pytest.raises(QuotientAIException) as exc_info:
        quotient_client.create_prompt_template(
            os.getenv("TEST_CREATE_PROMPT_TEMPLATE"), "Good template B"
        )
    assert "Prompt template with the same name already exists" in str(
        exc_info.value
    ), "Expected prompt template creation to fail with duplicate name"
