...
class TestHandleErrors:
    ...
    def test_authentication_error(self, caplog):
        ...
        assert result is None
        assert (
            "unauthorized: the request requires user authentication. ensure your API key is correct. Unauthorized"
            in caplog.text
        )
    ...
    def test_unprocessable_entity_error(self, caplog):
        ...
        def test_func(client):
            raise httpx.HTTPStatusError(
                "422 error", request=Mock(), response=mock_response
            )

        result = test_func(None)
        assert result is None
        assert (
            "Unprocessable entity error: missing required fields: required_field, another_field"
            in caplog.text
        )
    ...
    def test_connection_error(self, caplog):
        ...
        assert result is None
        assert (
            "connection error. please try again later. Connection failed" in caplog.text
        )
    ...
    def test_permission_denied_error(self, caplog):
        ...
        assert result is None
        assert (
            "forbidden: the server understood the request, but it refuses to authorize it. Forbidden"
            in caplog.text
        )
    ...
    def test_not_found_error(self, caplog):
        ...
        assert result is None
        assert (
            "not found: the server can not find the requested resource. Not Found"
            in caplog.text
        )
    ...
    def test_unexpected_status_code(self, caplog):
        ...
        def test_func(client):
            raise httpx.HTTPStatusError(
                "418 error", request=Mock(), response=mock_response
            )

        result = test_func(None)
        assert result is None
        assert (
            "Unexpected status code 418. contact support@quotientai.co for help. I'm a teapot"
            in caplog.text
        )
...
class TestErrorParsing:
    ...
    def test_parse_unprocessable_entity_error_without_detail(self, caplog):
        ...
        assert result is None
        assert (
            "API Response Validation Error: Missing detail in response" in caplog.text
        )
...
class TestAPIError:
    ...
    def test_api_error_init_with_dict_body(self):
        ...
        mock_request = Mock(spec=httpx.Request)
        body = {"code": "test_code", "param": "test_param", "type": "test_type"}
    ...
    def test_api_error_init_with_partial_dict_body(self):
        ...
        mock_request = Mock(spec=httpx.Request)
        body = {"code": "test_code", "type": "test_type"}
...
class TestAPIResponseValidationError:
    ...
    def test_api_response_validation_error_init_with_message(self):
        ...
        error = APIResponseValidationError(
            response=mock_response, body=body, message="Custom validation error"
        )
    ...
    def test_api_response_validation_error_init_without_message(self):
        ...
        error = APIResponseValidationError(response=mock_response, body=body)
    ...
    def test_api_response_validation_error_init_with_none_body(self):
        ...
        error = APIResponseValidationError(response=mock_response, body=None)
...
class TestAPIStatusError:
    ...
    def test_api_status_error_init_with_dict_body(self):
        ...
        error = APIStatusError(
            message="Custom error message", response=mock_response, body=body
        )
    ...
    def test_api_status_error_init_with_non_dict_body(self):
        ...
        error = APIStatusError(
            message="Server error message", response=mock_response, body=body
        )
    ...
    def test_api_status_error_init_with_none_body(self):
        ...
        error = APIStatusError(
            message="Not found message", response=mock_response, body=None
        )
...
class TestAPIConnectionError:
    ...
    def test_api_connection_error_init_with_custom_message(self):
        ...
        error = APIConnectionError(
            message="Custom connection error", request=mock_request
        )
