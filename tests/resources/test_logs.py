...
class TestLogsResource:
    ...
    def test_list_logs_with_dates(self, logs_resource, mock_client, sample_log_data):
        ...
        logs = logs_resource.list(start_date=start_date, end_date=end_date)
    ...
    def test_worker_thread_error_logging(self, logs_resource, caplog):
        ...
        error_logs = [
            ...
            if r.name == "quotientai.exceptions" and r.levelno == logging.ERROR
        ]
...
class TestAsyncLogsResource:
    ...
    async def test_list_logs(self, async_logs_resource, mock_client, sample_log_data):
        ...
        logs = await async_logs_resource.list(app_name="test-app", environment="test")
    ...
    @pytest.mark.asyncio
    async def test_list_logs_with_dates(
        self, async_logs_resource, mock_client, sample_log_data
    ):
        mock_client._get.return_value = {"logs": [sample_log_data]}
        ...
        logs = await async_logs_resource.list(start_date=start_date, end_date=end_date)
    ...
    @pytest.mark.asyncio
    async def test_list_logs_error_handling(
        self, async_logs_resource, mock_client, caplog
    ):
        mock_client._get.side_effect = Exception("API Error")
    ...
    async def test_post_log_in_background(self, async_logs_resource):
        ...
        await async_logs_resource._post_log_in_background(
            test_data
    ...
    @pytest.mark.asyncio
    async def test_cleanup_background_tasks_with_exception(
        self, async_logs_resource, mock_client, caplog
    ):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_background_tasks_with_empty_set(
        self, async_logs_resource, mock_client
    ):
